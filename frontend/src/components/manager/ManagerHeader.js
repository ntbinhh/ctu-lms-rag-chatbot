...
const ManagerHeader = () => {
  ...
  const items = [
    { label: "Trang chủ", icon: "pi pi-home", command: () => navigate("/manager") },
    {
      label: "Phòng học", icon: "pi pi-desktop",
      items: [
        { label: "Thêm phòng học", icon: "pi pi-plus", command: () => navigate("/manager/rooms/add") },
        { label: "Danh sách phòng học", command: () => navigate("/manager/rooms/list") },
      ],
    },
